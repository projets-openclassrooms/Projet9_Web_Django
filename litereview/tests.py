...
def test_delete_review(request, review_id):
    mydata = User.objects.all().values()
    template = loader.get_template("template.html")
    context = {
        'utilisateurs': mydata,
...
def test_update_review(request, review_id):
    mydata = User.objects.all().values()
    template = loader.get_template("template.html")
    context = {
        'utilisateurs': mydata,
...
@login_required
def test_follower_page(request):
...
@login_required
def test_ticket_page_update(request, ticket_id):
...
@login_required
def test_review_page_update(request, ticket_id, review_id):
    ticket = Ticket.objects.filter(id=ticket_id)[0]
    ticket_info = {}
    for field in ticket._meta.get_fields():
        excluded_field = ["id", "ticket", "review"]
...
@login_required
def test_tickets_reviews_page(request):
    ticket_form = forms.TicketForm()
    review_form = forms.ReviewForm()
...
@login_required
@require_POST
def test_unfollow_page(request):
    if request.method == 'POST':
...
def test_modify_ticket(request, ticket_id):
...
def test_modify_review(request, review_id):
...
def test_process_post_request(request):
...
def test_process_get_request(request):
